...
from microC.ast import dataclass

if TYPE_CHECKING:
    from .ast import Type, Value

T = TypeVar("T")
ScopeDict = dict[str, tuple["Type", "Value"]]
...
class Ctx:
    ...
    def __getitem__(self, name: str) -> "Value":
        ...
        if name in self.scope:
            return self.scope[name][1]  # retorna apenas o valor
    ...
    def get_type(self, name: str) -> "Type":
        """
        Obtém o tipo de uma variável pelo nome.
        """
        if name in self.scope:
            return self.scope[name][0]  # retorna apenas o tipo
        elif self.parent is not None:
    ...
    def __setitem__(self, name: str, value: "Value") -> None:
        ...
        """
        if name in self.scope:
            tipo, _ = self.scope[name]
            self.scope[name] = (tipo, value)
        elif self.parent is not None:
            self.parent[name] = value
        else:
            raise KeyError(f"Variable '{name}' not found in context.")

    def var_def(self, tipo: "Type", name: str, value: "Value") -> None:
        """
