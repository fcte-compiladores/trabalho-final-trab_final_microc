...
import microC.testing

EXAMPLES_PATH = Path(__file__).parent.parent / "exemplos"


def examples():
    yield from microC.testing.load_examples("")
    ...
    for subdir in sorted(EXAMPLES_PATH.iterdir()):
        ...
        yield from microC.testing.load_examples(mod, exclude=exclude)
...
def test_all(path: Path):
    ...
    example = microC.testing.Example(src, path)
